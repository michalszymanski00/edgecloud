...
// all subsequent calls reuse the same TLS config

export default client;

/** Fetch the list of devices (public). */
export function fetchDevices(): Promise<
  { id: string; last_seen: string }[]
> {
...
export function fetchTokens(): Promise<
...
export function registerDevice(
  deviceId: string,
  csrPem: string
): Promise<{ cert: string; ca: string }> {
  return axios
    .post<{ cert: string; ca: string }>(
      '/api/register',
      { device_id: deviceId, csr_pem: csrPem }
    )
    .then(r => r.data);
}
 ...
 */
export function fetchExpiringCerts(): Promise<
  { device_id: string; not_after: string }[]
> {
  return client
    .get('/admin/certs/expiring', { headers: adminHeaders })
    .then(r => r.data);
}
 ...
 */
export function fetchExpiredCerts(): Promise<
  { device_id: string; not_after: string }[]
> {
  return client
    .get('/admin/certs/expired', { headers: adminHeaders })
    .then(r => r.data);
}
 ...
 */
export async function fetchCertExpiry(): Promise<{
  expired: number;
